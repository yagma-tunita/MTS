...
1. 所有 pkg/ 模块添加了详尽的中文注释（包级说明 + 函数级解释 + 实现细节 + 设计理由）
2. net/（middleware, protect, router, websocket）添加了完整注释
3. internal/（handler, service, biz, notify）添加了完整注释
4. app/main.go 和 cmd/seed/main.go 添加了启动流程和种子数据注释
5. sql/ 建表和优化脚本添加了完整的中文业务注释
6. 订单查询接口（GetByID、列表、追踪）新增 Preload("City")，返回城市名称至前端
================================================================================
                               文档结束
================================================================================