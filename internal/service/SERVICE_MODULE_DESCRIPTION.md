...
- Delete(ctx, companyID int64) error
  软删除公司账号：先按 ID 查询公司是否存在（未删除的），存在则调用 DAO 的 Delete
  方法设置 delete_time = NOW()。只允许 admin 角色调用。
...
6. 账户管理增强
   - `ShipperCompanyService` 和 `ShippingCompanyService` 新增 `Delete(ctx, id)` 方法，
     实现管理员软删除货主/船运公司账号（设置 delete_time）。
